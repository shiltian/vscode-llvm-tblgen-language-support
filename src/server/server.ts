...
connection.onDidChangeConfiguration(async (change) => {
  ...
  if (config) {
    ...
    if (newCompilePath !== compileCommandsPath) {
      ...
      sendLog(
        `Configuration changed: compileCommandsPath = "${compileCommandsPath}"`,
      );
      await forceReindex();
    }
  }
...
async function buildIncludeGraph(): Promise<void> {
  ...
  try {
    ...
    if (!fs.existsSync(ccPath)) {
      sendLog(
        "Compile commands file not found. Symbol resolution will not work.",
      );
      sendLog(
        "Please set tablegen.compileCommandsPath or create tablegen_compile_commands.yml",
      );
...
async function ensureFilesIndexed(filePath: string): Promise<boolean> {
  ...
  sendLog(
    `Lazy indexing: ${toIndex.length} files for ${path.basename(filePath)}`,
  );
...
function registerCompositeDefmSymbols(parsed: ParsedFile, uri: string): void {
  const symbols = computeCompositeDefmSymbols(
    parsed.statements,
    uri,
    findMulticlassNode,
  );
...
connection.onDefinition(
  async (params: DefinitionParams): Promise<Location | null> => {
    ...
    if (isFieldAccessContext(doc, params.position)) {
      return resolveFieldAccessDefinition(
        doc,
        params.position,
        uri,
        visibleFiles,
    ...
    const parsed = parsedFiles.get(uri);
    const containingScope = parsed
      ? findScopeAtPosition(parsed, params.position)
      : undefined;
    ...
    if (containingScope) {
      const fieldDef = resolveLetBindingTarget(
        symbolName,
        containingScope,
        visibleUris,
      );
      if (fieldDef) {
        sendLog(
          `'${symbolName}' resolved to field definition in ${path.basename(fieldDef.uri)}`,
        );
        return fieldDef;
      }
    }
    ...
    sendLog(
      `Found ${validDefs.length} valid definitions for '${symbolName}' (filtered ${allDefs.length - validDefs.length} invalid)`,
    );

    for (const def of validDefs) {
      if (visibleUris.has(def.location.uri)) {
        sendLog(
          `Found definition (${def.kind}) in visible file ${path.basename(def.location.uri)}`,
        );
        return {
          uri: def.location.uri,
          range: def.location.range,
    ...
    sendLog(
      `Definition not found for '${symbolName}' in ${visibleFiles.length} visible files`,
    );
    return null;
  },
);
...
function resolveFieldAccessDefinition(
  ...
  if (text[objEnd - 1] === ">") {
    ...
    if (bracketCount === 0) {
      ...
      sendLog(
        `Field access: class instantiation ${className}<...>.${fieldName}`,
      );
    }
  } else {
    // Simple identifier
  ...
  const fieldInfo = typeSystem.findFieldDefinition(
    resolvedClassName,
    fieldName,
  );
  if (!fieldInfo) {
    sendLog(
      `Field access: field '${fieldName}' not found in '${resolvedClassName}'`,
    );
    return null;
  }
  ...
  sendLog(
    `Field access: found field '${fieldName}' in ${path.basename(fieldInfo.location.uri)}`,
  );
  return {
...
function findScopeInStatement(
  ...
  switch (stmt.type) {
    case "ClassDef":
      ...
      for (const bodyStmt of stmt.body || []) {
        if (
          bodyStmt.type === "LetStatement" &&
          isPositionInRange(position, bodyStmt.range)
        ) {
    ...
    if (scopeParts.length === 2) {
      ...
      if (classInfo) {
        ...
        for (const arg of classInfo.arguments) {
          if (arg.name === name) {
            sendLog(
              `Resolved '${name}' as template arg with type '${arg.type.name}'`,
            );
            return arg.type;
          }
        }
