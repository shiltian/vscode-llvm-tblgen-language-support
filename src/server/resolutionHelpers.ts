  ...
  for (const stmt of statements) {
    switch (stmt.type) {
      ...
      case "ForeachStatement": {
        const nested = findDefvarInStatements(
          stmt.body,
          defvarName,
          targetRange,
        );
      ...
      case "IfStatement": {
        const nestedThen = findDefvarInStatements(
          stmt.thenBody,
          defvarName,
          targetRange,
        );
        ...
        const nestedElse = findDefvarInStatements(
          stmt.elseBody,
          defvarName,
          targetRange,
        );
...
function collectDefSuffixes(
  ...
  for (const parent of mc.parentClasses) {
    ...
    if (parentMc) {
      for (const inherited of collectDefSuffixes(
        parentMc,
        findMulticlassNode,
        cache,
        visiting,
  ...
  for (const stmt of body) {
    if (stmt.type === "RecordDef" && stmt.name) {
      ...
      for (const parent of stmt.parentClasses) {
        ...
        if (nestedMc) {
          for (const sub of collectDefSuffixes(
            nestedMc,
            findMulticlassNode,
            cache,
            visiting,
  ...
  for (const stmt of statements) {
    if (stmt.type === "DefmDef" && stmt.name) {
      ...
      for (const parent of stmt.parentClasses) {
        ...
        if (mcDef) {
          const suffixes = collectDefSuffixes(
            mcDef,
            findMulticlassNode,
            suffixCache,
          );
